...
Le but du projet est de simuler et d'étudier des politiques d'aménagement des transports à l'aide d'un simulateur multi-agent. Nous adoptons une modélisation très simplifiée pour étudier quelques hypothèses et mesurer l'impact des paramètres sur la simulation.
...
\item \texttt{mainPackage} : contient la classe agent \texttt{Starter} qui permet de créer les agents pour lancer la simulation.
\end{itemize}
...
ADAPTER + PARLER DES PREENTRY

Afin de représenter les différents points utiles à notre simulation, nous créons les classes suivantes :
\begin{itemize}
...
ADAPTER ET DETAILLER LES WEIGHTS
...
\item \texttt{RoadPath} : chemin routier (agglomération) héritant de \texttt{Path} et comportant une méthode \texttt{weight()} retournant le temps de parcours nécessaire (pour une vitesse moyenne de 40 km/h) ;
...
Nous avons aussi créé une classe \texttt{Time} afin de représenter le temps et de pouvoir implémenter une horloge pour notre problème. Cette classe comporte deux attributs byte \texttt{hours} et \texttt{minutes}, ainsi que leurs getters. Elle comporte aussi les méthodes suivantes :
\begin{itemize}
...
\item \texttt{userPaths} : une \texttt{ArrayList} d'objets \texttt{Path} n'autorisant que les usagers non motorisés (\texttt{FootPath}, \texttt{BikePath} et TRANSPORTS EN COMMUN).
\end{itemize}
...
\item \texttt{findShortestPath} : implémente, à partir d'une \texttt{ArrayList<Point>}, d'une \texttt{ArrayList<Path>}, d'un point \texttt{source} et d'un point \texttt{target}, l'algorithme de Djikstra et retourne une \texttt{ArrayList<Point>} contenant les points formant le plus court chemin entre deux points ;
...
\item \texttt{findShortestUserPath} : retourne les points formant le plus court chemin pour un usager non motorisé ;
\item \texttt{shortestCarPath} : retourne les chemins formant le plus court chemin pour un usager automobiliste ;
\item \texttt{shortestUserPath} : retourne les chemins formant le plus court chemin pour un usager non motorisé.
...
\subsubsection{Lancement d'une simulation}
